...
def main():
    ...
    def parquet_scatter(
        ...
        if dataset_name == "airlines":
            ...
            x = (
                pd.to_numeric(df.get(xcol, 0), errors="coerce").fillna(0)
                + pd.to_numeric(df.get(x2, 0), errors="coerce").fillna(0)
            )
            ...
            save_scatter(
                ...
                y.tolist(),
                title=(
                    f"Airlines Parquet: Delayed+Cancelled vs Total (r={corr:.3f})"
                ),
                xlabel="Delayed+Cancelled",
                ylabel="Total",
                output_path=out_path / "airlines_corr_parquet.png",
            )
            ...
            save_scatter(
                x.tolist(),
                y.tolist(),
                title=f"Weather Parquet: Wind vs Precip (r={corr:.3f})",
                xlabel="Wind.Speed",
                ylabel="Precipitation",
                output_path=out_path / "weather_corr_parquet.png",
            )
        elif dataset_name == "video_games":
            xcol = (
                find_first_match(cols, ["review", "score"]) or
                find_first_match(cols, ["critic", "score"]) or
                find_first_match(cols, ["rating", "review"])
            )
            ycol = (
            ...
            save_scatter(
                x.tolist(),
                y.tolist(),
                title=f"VideoGames Parquet: Review vs Sales (r={corr:.3f})",
                xlabel="Review.Score",
                ylabel="Sales",
                output_path=out_path / "vg_corr_parquet.png",
            )
        elif dataset_name == "global_emissions":
            xcol = (
                find_first_match(cols, ["population"]) or find_first_match(cols, ["pop"])
            )
            ycol = (
            ...
            save_scatter(
                x.tolist(),
                y.tolist(),
                title=f"Emissions Parquet: Pop vs Emissions (r={corr:.3f})",
                xlabel="Population",
                ylabel="Emissions",
                output_path=out_path / "emissions_corr_parquet.png",
            )
    ...
    run_video_games(
        files["video_games"],
        CACHE / "video_games.parquet",
        OUT / "video_games",
    )
    run_emissions(
        files["global_emissions"],
        CACHE / "global_emissions.parquet",
        OUT / "emissions",
    )
    run_bd(

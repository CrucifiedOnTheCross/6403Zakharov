...
def _resolve_columns(cols: Iterable[str]) -> Dict[str, str]:
    ...
    return {
        "year": (
            ...
            find_first_match(cols, ["release", "year"])
        ),
        "sales": (
            find_first_match(cols, ["sales"]) or
            find_first_match(cols, ["metrics", "sales"])
        ),
        "publisher": (
            find_first_match(cols, ["publisher"]) or
            find_first_match(cols, ["company"]) or
            find_first_match(cols, ["developer"])
        ),
        "review": (
            find_first_match(cols, ["review", "score"]) or
            find_first_match(cols, ["rating", "review"]) or
            find_first_match(cols, ["critic", "score"])
        ),
        "rating": (
            find_first_match(cols, ["rating"]) or
            find_first_match(cols, ["content", "rating"])
        ),
    }
...
def run_all(csv_path: Path | str, parquet_path: Path | str, output_dir: Path | str):
    ...
    save_bar(
        [str(y) for y, _ in year_sorted],
        [v for _, v in year_sorted],
        title="VideoGames: Продажи по годам",
        ylabel="Sales",
        output_path=output_dir / "vg_sales_year.png",
    )

    pub_std = sorted(
        [(p, w.std) for p, w in publisher_review.items() if w.count > 1],
        key=lambda x: x[1],
    )
    save_bar(
        [p for p, _ in pub_std],
        [v for _, v in pub_std],
        title="VideoGames: Разброс оценок по издателям",
        ylabel="Std",
        output_path=output_dir / "vg_publisher_review_spread.png",
        rotation=90,
    )
    ...
    for rating, yr_map in rating_year.items():
        ...
        save_line(
            [str(y) for y, _ in ys],
            [c for _, c in ys],
            title=f"VideoGames: Кол-во игр рейтинга {rating}",
            ylabel="Count",
            output_path=output_dir / f"vg_rating_{rating}_count.png",
        )
    ...
    save_scatter(
        corr_x,
        corr_y,
        title=f"VideoGames: Review vs Sales (r={corr:.3f})",
        xlabel="Review.Score",
        ylabel="Sales",
        output_path=output_dir / "vg_corr.png",
    )

    summary = [
        f"Лучший год по продажам: {best_year}",
        f"Худший год по продажам: {worst_year}",
        "Издатели с минимальным разбросом: "
        + ", ".join([f"{p}:{v:.3f}" for p, v in pub_std[:3]]),
        "Издатели с максимальным разбросом: "
        + ", ".join([f"{p}:{v:.3f}" for p, v in pub_std[-3:][::-1]]),
